...
[Mixcloud archive](https://mixcloud.com/$MC_USERNAME/playlists/$HOST_MC_PLAYLIST)"
  ...
  # Look up hero_focus from data/hero-focus.json (generated by detect-faces.py)
  HERO_FOCUS=""
  if [[ -f "data/hero-focus.json" ]]; then
    HERO_FOCUS=$(jq -r --arg slug "$ARTIST_FILENAME" '.[$slug] // ""' data/hero-focus.json)
  fi
